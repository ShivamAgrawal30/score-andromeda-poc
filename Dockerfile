...
WORKDIR /go/src/github.com/score-spec/score-andromeda
...
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -o /usr/local/bin/score-andromeda ./cmd/score-andromeda
...
# Set the current working directory inside the container.
WORKDIR /score-andromeda

# Copy the binary from the builder image.
COPY --from=builder /usr/local/bin/score-andromeda /usr/local/bin/score-andromeda

# Run the binary.
ENTRYPOINT ["/usr/local/bin/score-andromeda"]